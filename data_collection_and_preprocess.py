...
def remove_latex_commands(text: str) -> str:
    """Remove LaTeX commands while preserving some formatting and handling special cases.
       Extended to handle complex formulas like \frac{}{}, \sum, \int, etc."""
    ...
    for line in lines:
        ...
        line = re.sub(r'\\frac\{.*?\}\{.*?\}', ' ', line)  # \frac{num}{denom}
        line = re.sub(r'\\sqrt\{.*?\}', ' ', line)         # \sqrt{content}
        line = re.sub(r'\\(?:sum|int|prod)(?:\s*\^.*?)?\{.*?\}', ' ', line)  # \sum_{i=1}^n, \int_a^b
        line = line.replace('\\', ' ')
